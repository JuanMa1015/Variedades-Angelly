  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      try {
        ...
        const requestList = [
          fetch(`${API_BASE_URL}/api/dashboard/resumen`, {
        ...
        const resumenPayload = await resumenResponse.json().catch(() => ({}));
        const ventasPayload = await ventasResponse.json().catch(() => ([]));
        const carteraPayload = carteraResponse
          ? await carteraResponse.json().catch(() => ([]))
          : [];

        if (!resumenResponse.ok) {
          throw new Error(resumenPayload.detail || 'No fue posible cargar el resumen de ventas');
        }
        ...
        if (isAdmin && carteraResponse && !carteraResponse.ok) {
          throw new Error('No fue posible cargar el resumen de cartera');
        }
