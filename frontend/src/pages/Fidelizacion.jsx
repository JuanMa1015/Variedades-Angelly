...
const Fidelizacion = () => {
  ...
  const fetchClientes = async (signal) => {
    const response = await fetch(`${API_BASE_URL}/api/fidelizacion/clientes`, {
      signal,
      headers: {
    ...
    if (!response.ok) {
      throw new Error('No se pudo cargar la base de fidelización');
    }
  ...
  const handleCanjearBono = async (cliente) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fidelizacion/clientes/${cliente.id}/canjear-bono`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        throw new Error(payload.detail || 'No se pudo canjear el bono');
      }
