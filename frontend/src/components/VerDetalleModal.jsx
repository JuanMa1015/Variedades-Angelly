...
const VerDetalleModal = ({ cliente, isOpen, onClose, apiBaseUrl, token }) => {
  ...
  useEffect(() => {
    if (!isOpen || !cliente?.id || !apiBaseUrl) return;
    ...
    const fetchMovimientos = async () => {
      try {
        ...
        const params = new URLSearchParams({
          ...
          limit: String(MOVIMIENTOS_PAGE_SIZE),
        });

        const response = await fetch(
          `${apiBaseUrl}/api/clientes/${cliente.id}/movimientos?${params.toString()}`,
          {
  ...
  }, [apiBaseUrl, cliente?.id, currentPage, isOpen, token]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-5 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-gray-600 text-sm">Límite Crédito</p>
            <p className="text-2xl font-bold text-gray-900">
              ${Number(cliente.limite_credito ?? 0).toLocaleString('es-CO')}
            </p>
          </div>
