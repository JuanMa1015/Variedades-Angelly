const ClienteCarteraCard = ({
  ...
  // Calcular niveles de deuda para color
  const deudaActual = Number(cliente.deuda_total ?? 0);
  const limiteCredito = Number(cliente.limite_credito ?? 0);
  const usagePercent = limiteCredito > 0 ? (deudaActual / limiteCredito) * 100 : 0;
  ...
  if (deudaActual === 0) {
    ...
    indicatorBg = '#fffbeb';
    indicatorLabel = 'Atención';
  } else {
    indicatorColor = '#ef4444';
    indicatorBg = '#fef2f2';
    indicatorLabel = 'Crítico';
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-shadow border border-gray-100 h-full">
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div
          className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex flex-col items-center justify-center text-center shrink-0"
          style={{ backgroundColor: indicatorBg }}
        >
          <div className="text-lg sm:text-2xl font-bold" style={{ color: indicatorColor }}>
            {usagePercent > 0 ? Math.round(usagePercent) : 0}%
          </div>
          <p className="text-xs font-semibold" style={{ color: indicatorColor }}>
            {indicatorLabel}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4 mb-6">
        <div className="bg-gray-50 rounded-lg p-4">
          <p className="text-gray-600 text-sm font-semibold uppercase mb-1">Límite de Crédito</p>
          <p className="text-2xl sm:text-3xl font-bold text-gray-900">${limiteCredito.toLocaleString('es-CO')}</p>
        </div>

        <div className="bg-gray-50 rounded-lg p-4">
          <p className="text-gray-600 text-sm font-semibold uppercase mb-1">Total Compras (Fiado)</p>
          <p className="text-2xl sm:text-3xl font-bold text-gray-900">${deudaActual.toLocaleString('es-CO')}</p>
        </div>
