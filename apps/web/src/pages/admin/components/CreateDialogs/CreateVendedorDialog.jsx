...
const CreateVendedorDialog = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await apiRequest('/api/superadmin/usuarios/vendedores', {
        method: 'POST',
        body: { username: form.username.trim(), password: form.password },
      });
      setForm({ username: '', password: '' });
      onCreated('Vendedor creado');
    } catch (err) {
      setError(err.message || 'No se pudo crear vendedor');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} variant="admin" maxWidth="max-w-2xl" title="Crear vendedor" subtitle="Alta rápida desde modal.">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
          ...
          <button type="submit" disabled={saving} className="rounded-full bg-blush-300 px-4 py-2 text-sm font-semibold text-rosewood transition hover:bg-blush-300">{saving ? 'Guardando…' : 'Crear vendedor'}</button>
        </div>
      </form>
    </Modal>
  );
...
export default CreateVendedorDialog;
